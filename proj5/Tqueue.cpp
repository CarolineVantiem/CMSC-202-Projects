...
Tqueue<T>::Tqueue(const Tqueue<T>& x) {
  ...
  for (int i = x.m_front; i < x.m_back; i++) {
    m_data[i] = x.m_data[i];
  }
}
...
void Tqueue<T>::enqueue(T data) {
  ...
  // if queue is at max capacity
  else if (m_back == m_capacity) {
...
void Tqueue<T>::dequeue() {
  ...
  // queue size is half cap
  if (size() <= m_capacity / 2) {
    T *temp3 = new T[m_capacity / 2];
    for (int i = 0; i < m_back; i++) {
      temp3[i] = m_data[i + m_front];
    }
    delete [] m_data;
    m_data = temp3;
    m_capacity = m_capacity / 2;
  ...
  // all other queue sizes
  else {
    T *temp4 = new T[m_capacity];
    for (int i = 0; i < m_back; i++) {
      temp4[i] = m_data[i + m_front];
    }
    delete [] m_data;
    m_data = temp4;
    m_back--;
